    ...
    return __async(this, null, function* () {
      ...
      try {
        ...
        if (response.data.code === 1) {
          ...
          yield Swal.fire("Error", "Failed to load enquiry list", "error");
        }
      } catch (error) {
        console.error("Error loading enquiry list:", error);
        yield Swal.fire("Error", "An error occurred while loading the enquiry list", "error");
      } finally {
        this.isLoading = false;
      }
    });
    ...
    return __async(this, null, function* () {
      try {
        ...
        if (response.data.code === 1) {
          ...
          yield Swal.fire("Success", response.data.message, "success");
        } else {
          console.error("Failed to toggle status:", response.data.message);
          yield Swal.fire("Error", "Failed to update status", "error");
        }
      } catch (error) {
        console.error("Error toggling status:", error);
        yield Swal.fire("Error", "Error updating status", "error");
      }
    });
  }
...
//# sourceMappingURL=enquiry.component-4UCRF2MI.mjs.map
